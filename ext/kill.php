...
if ($chan[0] == "#") {
	...
	elseif ($toys == "1") {
		echo("NOTICE $nick :Toys must be activated publicly at \002$chan\002 to use killing.\n");
	}
	elseif ($toys == "2") {
		$exp = explode(" ",$params);
		...
		if ($params == "") {
			...
			if (count($bla) > 5) {
				echo("NOTICE $nick :killing more than \0025\002 users is not allowed.\n");
				return(0);
			}
			...
			foreach ($bla as $xx) {
				if ($nfound[strtolower($xx)] == "") {
					...
					if ($slapd[strtolower($xx)] == 1) {
						echo("NOTICE $nick :You already included \002".$nfound[strtolower($xx)]."\002 to your kill attack.\n");
						return(0);
					}
